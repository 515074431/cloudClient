...
const checkingFiles = {
    ...
    if (oldDirIndex == -1) {//文件夹不存在  直接创建
      ...
      if (currentDir.etag != oldDir.etag) {//etag有变化
        ...
        if (currentDir.type == 'file') {//是文件只需要重新下载
          ...
          const currentChildren = await this.webDavClient.getDirectoryContents(checkDir, { deep: 1, filterSelf: true });
          ...
          for (let currentChild of currentChildren) {//先根据远程有的文件同步到本地文件
            ...
            if (oldChildIndex == -1) {//本地没有，没有则下载,有则要检查是否一致
              ...
              await this.wGetRemote2local(currentChild);//下载文件或文件夹到本地
            } else {//本地有对应文件或文件夹
   ...
   * @returns {Promise<*>}
   */
  firstDownLoad: async function() {
    let filesData = await this.webDavClient.getDirectoryContents(this.remotePath, {
      deep: 'infinity',
      filterSelf: false,
    });
  ...
  init: async function(remotePath, localPath) {
    ...
    if (Object.keys(this.userInfo).length == 0) {
      console.log('没有获得用户信息，需要重新设置用户信息');
      this.userInfo = {
        username: 'test',
        password: 'test1234',
        remoteUrl: 'http://localhost:8080',
        token: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJhdWQiOiJ0ZXN0IiwiZXhwIjoxNTczMTkxNTg2LCJqdGkiOiIxNyIsImlhdCI6MTU3MzEwNTE4NiwiaXNzIjoiZ2luLWFudGQiLCJuYmYiOjE1NzMxMDUxODYsInN1YiI6ImxvZ2luIn0.MTdCn8eN0H2UtYB7MtPr7SdoUoJEt8aVpT9IilFJRs4',
      };
      await storage.set('userInfo', this.userInfo, { dataPath: dataPath });
    ...
    this.filesData = await storage.get('filesData', { dataPath: dataPath });//本地数据信息
    //console.log('文件信息filesData',filesData)
    if (Object.keys(this.filesData).length == 0) {
      console.log('没有filesData，第一次下载');

      this.filesData = await this.firstDownLoad();
      //console.log('下载后：',filesData)
      await storage.set('filesData', this.filesData, { dataPath: dataPath });//存储数据
    }
    console.log('不是第一次下载：');
